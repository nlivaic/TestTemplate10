param ($environment, $projectName, $resourceGroupName, $sqlAdminUsername, $sqlAdminPassword, $tenantId, $authAuthority, $authAudience, $authValidIssuer)

Write-Host "##[warning]------ Create Resources START ------"
$jsonResultAll = az deployment group create --resource-group $resourceGroupName --template-file ./deployment/iac.bicep --parameters environment=$environment projectName=$projectName sql_admin_username=$sqlAdminUsername sql_admin_password=$sqlAdminPassword tenantId=$tenantId authAuthority=$authAuthority authAudience=$authAudience authValidIssuer=$authValidIssuer | ConvertFrom-Json
